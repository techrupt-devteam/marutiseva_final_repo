...
class BookingController extends Controller
{
    ...
    public function offers()
    {
        ...
        $data     = \DB::table('offers')
                    ->join('product_cat','offers.car_maker','=','product_cat.id')
                    ->select('product_cat.car as car_name','offers.*')
                    ->get();
...
/*        $data     = \DB::table('nexa_scheduled_value_added_service')
                        ...
                        ->get();
*/
         $data =$this->get_list('nexa_scheduled_value_added_service','yes');
    ...
    public function enquiry()
    {
        ...
        $data     = \DB::table('enquiries')
                    ->Leftjoin('product_cat','enquiries.car','=','product_cat.id')
                    ->select('product_cat.car as car_name','enquiries.*')
                    ->orderBy('id','DESC')
                    ->get();
...
public function commercial_feedback()
    ...
    public function getvarientnexa(Request $request)
    {
       $id = $request->id;
    ...
    public function getvarientcomm(Request $request)
    {
        $id =  $request->id;
        if ($id != null) {
    ...
    public function getcolornexa(Request $request)
    {
        $varient = $request->varient;
        $city = \DB::table('nexa_product')
                ->where(['varient'=>$request->varient])
    ...
    public function getcolorcomm(Request $request)
    {
    ...
    public function add_offer(Request $request)
    {
        $car = \DB::table('product')
               ->join('product_cat','product.car','=','product_cat.id') 
               ->select('product_cat.car','product_cat.id as car_id')
               ->groupBy('product.car')
               ->get();
        ...
        if(isset($_FILES["image"]["name"]) && !empty($_FILES["image"]["name"]))
            {
                ...
                $arr_data['image']                 = '/admin/upload/offers/'.$img_name;
        ...
        if(isset($_FILES["image"]["name"]) && !empty($_FILES["image"]["name"]))
            {
              ...
              /*  if(move_uploaded_file($file_tmp,env('BASE_PATH').$latest_image))
                ...
                $file                              = $request->file('image');
                $img_name                          = $randomString."-nexa-".$file->getClientOriginalName();
                $arr_data['image']                 = '/admin/upload/offers/'.$img_name;
            } 
        // dd($arr_data);
        ...
        if(isset($_FILES["image"]["name"]) && !empty($_FILES["image"]["name"]))
            {
                ...
                $file                              = $request->file('image');
                $img_name                          = $randomString."-comm-".$file->getClientOriginalName();
                $arr_data['image']                 = '/admin/upload/offers/'.$img_name;
    ...
    public function update_nexa_offer(Request $request,$id)
   {
    ...
    {   
        ...
        foreach ($data as $key => $value) 
        {
              echo $key+1;
              echo ',';
              echo date('d-m-Y',strtotime($value->test_drive_date));
              echo ',';
              echo $value->test_drive_time;
              echo ',';
              ...
              echo $value->phone_no;
              echo ',';
              echo ucfirst($value->city);
              echo ','; 
              echo ucfirst($value->model);
              echo ',';
              echo "\n";
        }
        //print_r($data);
    ...
    //seva enquiry csv report 
    public function seva_enq_csv(Request $request)
    {   
        $from_date = date('Y-m-d',strtotime($request->input('from_date')));
        $to_date   = date('Y-m-d',strtotime($request->input('to_date')));

        $data  = DB::select('select * from enquiries where date(created_at) >= "'.$from_date.'" AND date(created_at) <="'.$to_date.'"');  
        
        $from_date = date('d-m-Y',strtotime($request->input('from_date')));
        $to_date   = date('d-m-Y',strtotime($request->input('to_date')));              
        
        $file_name = "Enquiry Rpeort Date From ".$from_date." To ".$to_date;
    ...
    //nexa test drive csv report 
    public function nexa_test_download_csv(Request $request)
    {   
        $from_date = date('Y-m-d',strtotime($request->input('from_date')));
        $to_date   = date('Y-m-d',strtotime($request->input('to_date')));
        $data      = \DB::table('nexa_test_drive')
        ...
        $from_date = date('d-m-Y',strtotime($request->input('from_date')));
        $to_date   = date('d-m-Y',strtotime($request->input('to_date')));              
        $file_name = "Nexa Test Drive Date From ".$from_date." To ".$to_date;
        header("Content-type: text/csv");
        header("Content-Disposition: attachment; filename=".$file_name.".csv");
        header("Pragma: no-cache");
        header("Expires: 0");
    ...
    //nexa enquiry csv report 
    public function nexa_enq_csv(Request $request)
    {   
        $from_date = date('Y-m-d',strtotime($request->input('from_date')));
        $to_date   = date('Y-m-d',strtotime($request->input('to_date')));

        $data  = DB::select('select * from nexa_enquiries where date(created_at) >= "'.$from_date.'" AND date(created_at) <="'.$to_date.'"');  
        
        $from_date = date('d-m-Y',strtotime($request->input('from_date')));
        $to_date   = date('d-m-Y',strtotime($request->input('to_date')));              
        
        $file_name = "Nexa Enquiry Rpeort Date From ".$from_date." To ".$to_date;
     ...
     //commercial enquiry csv report 
    public function comm_enq_csv(Request $request)
    {   
        ...
        $data  = DB::select('select * from commercial_enquiries where date(created_at) >= "'.$from_date.'" AND date(created_at) <="'.$to_date.'"');  
        
        $from_date = date('d-m-Y',strtotime($request->input('from_date')));
        $to_date   = date('d-m-Y',strtotime($request->input('to_date')));              
        
        $file_name = "Commercial Enquiry Rpeort Date From ".$from_date." To ".$to_date;
